...
p1 = detectors |>
  ...
  scale_fill_manual(values=c("#9A1E5B","#F15B47"),labels=c("Real TOEFL Essays","Real US 8th-Grade Essays")) +
  coord_cartesian(clip="off") +
  theme1 +
  labs(y="Detector", x=g1a)

p2 = detectors |>
  filter(name %in% c("Fake TOEFL - GPT4 - PE","US 8th grade essay - GPT simplify")) |>
  mutate(re = case_when(kind==.pred_class~1,TRUE~0)) |>
  group_by(name, detector) |>
  count(re) |> ungroup() |>
  group_by(detector, name) |>
  ...
  geom_text(aes(label=scales::percent(prop, accuracy=1)), vjust=-1, family=f1, show.legend = FALSE, size=2.7) +
  scale_x_continuous(limits=c(0,1), labels=scales::percent, expand=c(0.02,0.02)) +
  scale_fill_manual(values=c("#3d0066","#c670ff"),labels=c("Real TOEFL Essays + Enhanced Word Choices","Real US 8th-Grade Essays + Simplified Word Choice")) +
  coord_cartesian(clip="off") +
  theme1 +
  labs(y="Detector", x=g1a)
```
...
```{r plots 3 and 4}
# simple prompts
# reference: page 7, https://arxiv.org/pdf/2304.02819.pdf
g1b = "Correctly Classified as AI-Generated"
p3 = detectors |>
  filter(name %in% c("Fake College Essays - GPT3","Fake College Essays - GPT3 Prompt Engineered")) |>
  mutate(re = case_when(kind==.pred_class~1,TRUE~0)) |>
  group_by(name, detector) |>
  count(re) |> ungroup() |>
  group_by(detector, name) |>
  ...
  geom_text(aes(label=scales::percent(prop, accuracy=1)), vjust=-1, family=f1, show.legend = FALSE, size=2.7) +
  scale_x_continuous(limits=c(0,1), labels=scales::percent, expand=c(0.02,0.02)) +
  scale_fill_manual(values=c("#293087","#4293CF"),labels=c("US College Admission Essays: ChatGPT-3.5 generated","ChatGPT-3.5 generated w/ prompt design")) +
  coord_cartesian(clip="off") +
  theme1 +
  labs(y="Detector", x=g1b)

p4 = detectors |>
  filter(name %in% c("Fake CS224N - GPT3","Fake CS224N - GPT3, PE")) |>
  mutate(re = case_when(kind==.pred_class~1,TRUE~0)) |>
  group_by(name, detector) |>
  count(re) |> ungroup() |>
  group_by(detector, name) |>
  ...
  geom_text(aes(label=scales::percent(prop, accuracy=1)), vjust=-1, family=f1, show.legend = FALSE, size=2.7) +
  scale_x_continuous(limits=c(0,1), labels=scales::percent, expand=c(0.02,0.02)) +
  scale_fill_manual(values=c("#003E2B","#6BC84E"),labels=c("Scientific Abstracts:ChatGPT-3.5 generated","ChatGPT-3.5 generated w/ prompt design")) +
  coord_cartesian(clip="off") +
  theme1 +
  labs(y="Detector", x=g1b)
```
