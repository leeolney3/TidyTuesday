...
ggplot() +
  geom_sf(data=outline, aes(geometry=geometry), size=.2, color="#4a4e69") +
  geom_sf(data=voronoi, aes(geometry=geometry), size=.2, show.legend = FALSE, color="#4a4e69", alpha=.9, fill="white") +
  geom_sf(data=data, shape=21, fill="#eb5e28", color="white", size=1, stroke=.3) +
  scale_fill_manual(values=pal) +
  coord_sf(xlim=c(-84.82016,-80.51889), expand=FALSE) +
  cowplot::theme_map(10) +
  theme(plot.title=element_text(family=f1, face="plain", color="white")) +
  labs(title = s)
}
...
patchwork +
  plot_annotation(title="Bigfoot Sightings in Ohio",
                  ...
                  theme=theme(plot.title=element_text(face="bold", family=f1, color="white"),
                              plot.subtitle=element_text(family=f2, color="white", size=9.5),
                              plot.caption = element_text(size=8,hjust=0, color="grey95", family=f2),
