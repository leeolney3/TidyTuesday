...
plot1 = snack_tab %>% 
  ...
  scale_x_date(limits=c(min(snack_tab$date_aired), max(snack_tab$date_aired)+3000)) + 
  scale_color_manual(values=c("#966a00","#79af30","#622486","#F7801E","#76a2ca")) +
...
#### Character and action sankey chart
...
#### Amount (suspects, culprit, monster)
...
amount %>%
  ...
  scale_x_date(limits=c(min(amount$date_aired), max(amount$date_aired)+3000)) + 
...
### Modeling
#### Predict which monster is real
This section of the notebook follows [Julia Silge's screencast](https://juliasilge.com/blog/scooby-doo/) on using decision tree to predict if the monsters in Scooby Doo are real or not. 
...
# theme set and update
theme_set(theme_minimal(base_size = 10)) 
theme_update(panel.grid.minor=element_blank())
```

```{r}
# Proportion of real monsters
sd_monster = scoobydoo %>% filter(monster_amount > 0)
Hmisc::describe(sd_monster$monster_real)
```
...
sd1 = sd_monster %>%
  ...
  ggplot(aes(fct_rev(year_aired), n, fill = monster_real)) +
  geom_col(position = position_dodge(preserve = "single"), alpha = 0.7,width=0.5) +
  coord_flip() +
  scale_fill_manual(values=c("#ee9b00","#457b9d"),guide = guide_legend(reverse = TRUE)) +
  scale_y_continuous(breaks=seq(0,125,25),expand=c(0.03,0.03)) + 
  labs(x = "Date aired", y = "Monsters per decade", fill = "Real monster?")

# imdb distribution by monsters_real
sd2 = sd_monster %>%
...
sd1 + sd2 + plot_layout(guides = "collect") & theme(legend.position = "top")
```
...
```{r}
# plot combinations
autoplot(tree_rs) + theme_light() + scale_color_jco()
```
```{r}
# simpler model that is within one standard error of the optimal results
...
scooby_train %>%
  ...
  scale_fill_manual(values=c("#ee9b00","#457b9d"),guide = guide_legend(reverse = TRUE)) + 
  scale_color_manual(values=c("#ee9b00","#457b9d"),guide = guide_legend(reverse = TRUE))
```

#### Predict IMDB score
This section of the notebook follows [Eric Ekholm's blog post post](https://www.ericekholm.com/posts/2021-07-20-scooby-doo-eda/) on using linear models to predict the IMDB score of an episode. 
...
miss_tab = as.data.frame(t(miss_df)) %>% 
  rename(missing_rate=V1) %>%
  arrange(desc(missing_rate)) %>% filter(missing_rate>0) 

miss_tab
```

```{r}
# drop variables with >0.2 missing rate
keep_vars <- miss_df %>%
...
```{r}
# bootstrap resamples set up
set.seed(0408)
booties <- bootstraps(mod_df, times = 10)

# basic processing using recipe
...
mod_fit %>%
  ...
  geom_col() +
  scale_fill_manual(values=c("#ee9b00","#457b9d")) +
  theme(plot.title.position = "plot") +
  labs(y = NULL, title="Lasso regression: coefficients") 
```
